...
const AdminLoginModal: React.FC<AdminLoginModalProps> = ({ isOpen, onClose, onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        // We use the email/pass method, but passing user/pass for admin hardcoded check in mock api
        const user = await api.login('admin@bacphysique.tn', password);
        
        // Extra check to ensure it's actually an admin (in case someone registered with that email)
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden z-10 animate-[fadeIn_0.3s_ease-out] border border-brand-100">
        ...
        <div className="p-8">
            <div className="flex flex-col items-center mb-6">
                ...
                <h3 className="text-2xl font-bold text-brand-900">Espace Privé</h3>
                <p className="text-brand-500 text-sm">Accès réservé aux enseignants</p>
            </div>
